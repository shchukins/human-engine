...
Текущая web-реализация доступна как FastAPI SSR surface на `/today`. Она
работает без отдельного iOS-приложения и использует backend как единственный
источник readiness и recommendation.
...
Пользователь открывает web-экран с телефона и хочет за 5–10 секунд понять:
	1.	в каком он состоянии сегодня
	2.	хороший ли это день для тренировки
	3.	что именно повлияло на это состояние
	4.	одним нажатием отметить recovery и RPE последней тренировки
...
В первой версии не делаем:
	...
	•	дополнительные обязательные вопросы о сне, стрессе или мотивации
	•	frontend-side readiness/recommendation logic
